...
function GWindow(a1, a2, a3) {
   ...
   function _keydown(e) {
      let fn = callbacks["keydown"];
      if (fn) fn(e);
   }

   function _keyup(e) {
      let fn = callbacks["keyup"];
      if (fn) fn(e);
      fn = callbacks["keypress"];
      if (fn) fn(e);
   }
...
/* Protected methods */
   ...
   function _paint(ctx) {
      if (visible) {
         ...
         if (!gobj._isTransformed() && gobj._paintDirectly) {
            gobj._paintDirectly(ctx, x, y);
         } else {
            ctx.translate(x, y);
...
/*
 * Method: getTextAlign
 * Usage: let textAlign = label.getTextAlign();
 * --------------------------------
 * Gets the horizontal text alignment of this label. Valid values
 * are "right" (the right of the text will be aligned with the x,y coordinates),
 * "center", "left", and "start" (defaultâ€“ left for left-to-right, right for right-to-left).
 */
   glabel.getTextAlign = function () {
      return textAlign;
   }

/*
* Method: setTextAlign
* Usage: label.setTextAlign("top");
* --------------------------------
* Sets the baseline of this label (i.e. its vertical alignment). Valid values
* are "top" (the top of the text will be aligned with the x,y coordinates),
* "middle", "alphabeteic" (default), and "bottom".
*/
   glabel.setTextAlign = function (newTextAlign) {
...
/*
 * Method: getBaseline
 * Usage: let baseline = label.getBaseline();
 * --------------------------------
 * Gets the baseline of this label (i.e. its vertical alignment). Valid values
 * are "top" (the top of the text will be aligned with the x,y coordinates),
 * "middle", "alphabeteic" (default), and "bottom".
 */
   glabel.getBaseline = function () {
      return baseline;
   }

/*
 * Method: setBaseline
 * Usage: label.setBaseline("top");
 * --------------------------------
 * Sets the baseline of this label (i.e. its vertical alignment). Valid values
 * are "top" (the top of the text will be aligned with the x,y coordinates),
 * "middle", "alphabeteic" (default), and "bottom".
 */
   glabel.setBaseline = function (newBaseline) {
      baseline = newBaseline;
      glabel.repaint()
   }

   glabel._paintDirectly = function(ctx, x, y) {
      ctx.font = fontTag;
 ...
 */
   ...
   gpoly._paintDirectly = function(ctx, x, y) {
      ctx.beginPath();
 ...
 */
   ...
   //goval._paintDirectly = function(ctx, x, y) {
   //   ctx.translate(width / 2, height / 2);
   //   ctx.scale(1, height / width);
   //   ctx.beginPath();
 ...
 */
   ...
   grect._paintDirectly = function(ctx, x, y) {
      if (grect.isFilled()) {
 ...
 */
   ...
   gline._paintDirectly = function(ctx, x, y) {
      ctx.beginPath();
      ctx.moveTo(x, y);
...
/* Protected methods */
   ...
   gimage._paintDirectly = function(ctx, x, y) {
      if (this._image.complete) {
